...
public class JDKInitActivator extends Plugin {
	...
	/**
	 * Returns the home directory of a JDK that can be used to use the
	 * jdk.attach package. 1) First checks the running JVM: if it is a JDK,
	 * returns the java.home 2) Second checks if the
	 * de.schenk.toolsjar\bin\java9 folder contains a valid JDK (java.exe in
	 * bin\java.exe) and returns if available.
	 * 
	 * @return
	 */
	private File findJDK9HomeDir() {
		final String version = System.getProperty("java.version");
		if (version.startsWith("9") || version.startsWith("1.9")) {
			final String javahome = System.getProperty("java.home");

			File jdkbin = new File(javahome + File.separator + "bin");
			if (new File(jdkbin, "attach.dll").exists() && new File(jdkbin, "java.exe").exists())
	...
	private File findIncludedJavaSpecificFiles(String filename) {
		...
		File toolsjar = new File(alternateToolsJarSources + "/" + javaversion + "/" + filename);
	 ...
	 * 
	 * @return true if a JDK9 is available, false if a JDK8 or below is
	 *         available.
	 * 
	 */
